...
const countWords = (wurdz) => {
  wurdz = wurdz.trim().toLowerCase();
  const wordReg = /[^'\s][\w'?]*[^\:\s\,']/gi;
  const regArray = [];
  let wordObject = {};

  regArray.push(...wurdz.matchAll(wordReg));
  for (let x = 0; x < regArray.length; x++) {
    if (Object.hasOwn(wordObject, regArray[x][0])) continue;

    const tempReg = new RegExp(String.raw`\b${regArray[x][0]}\b`, "g");
    let wordNum = [...wurdz.matchAll(tempReg)].length;
    Object.assign(wordObject, { [regArray[x][0]]: wordNum });
  }
  return wordObject;
};
